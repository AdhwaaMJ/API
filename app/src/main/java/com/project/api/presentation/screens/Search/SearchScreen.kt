            ...
            LazyVerticalGrid(
                ...
                items(moviePagingItems.itemCount) {index ->
                    if (moviePagingItems[index]?.adult==false){
                        AsyncImage(model = "${Constant.MOVIE_IMAGE_BASE_URL}${BackdropSize.w300}/${moviePagingItems[index]?.posterPath}",
                            contentDescription = "",
                            modifier = Modifier
                                .padding(2.dp)
                                .clickable {
                                    navController.navigate(AppScreen.MoviesDetailsScreen.rout + "/${moviePagingItems[index]?.id}")
                                },
